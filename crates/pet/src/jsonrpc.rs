...
pub fn start_jsonrpc_server() {
    ...
    // let cache = Some(Arc::new(Cache::new(PathBuf::from(
    //     "/Users/donjayamanne/Development/vsc/python-environment-tools/temp/cache",
    // ))));
    let cache_reporter = CacheReporter::new(jsonrpc_reporter.clone(), None);
    let conda_locator = Arc::new(Conda::from(&environment));
    let context = Context {
        cache: None,
        reporter: Arc::new(cache_reporter),
